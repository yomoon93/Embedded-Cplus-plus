...
void loop()
{
  ...
  { 
    ...
    {
      Serial.println("captions");                // send "captions" serially.
    }
  }
  ...
  { 
    ...
    {
       find_distance();
       if(distance2<=15 && distance2>=1)  // if our hand detects in the left sensor before 300 milli seconds this condition becomes true. ( usually it happens if we swipe our hand from left to right sensor )
       {
         Serial.println("rewind");         // send "change" serially.
    ...
    {
    Serial.println("fast foward");            // send "fast foward" serially.
    while(distance1<=15 && distance1>=1) // this loop will rotate untill we removes our hand infront of the left sensor. this will avoid not to enter this if condition again.
    find_distance();                      
    }
    L=0;                                  // make l=0 for the next round.
   }
    temp=distance1;                         // store the current position of our hand in the variable temp. 
     while(distance1<=0 || distance1==15)    // this loop will run untill we removes our hand in front of the right sensor.
     {
       find_distance();                      // call this function continuously to get the live data. 
       if((temp+6)<distance2)                // this condition becomes true if we moves our hand away from the right sensor (**but in front of it ). here " temp+6 " is for calibration.
       {
       Serial.println("playpause");               // send "expand" serially.
       }
